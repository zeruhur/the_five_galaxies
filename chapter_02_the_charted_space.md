...
Oddspace and WAN travel are distinct and complementary. WAN corridors offer faster, more reliable transit between nodes but require access to maintained infrastructure and the approval of the operating power. Oddspace travel is slower and more demanding but does not require permission. The practical consequence is that WAN-controlling polities can deny transit rights to specific vessels; those vessels can use oddspace to bypass the WAN, at higher fuel cost and navigational risk. This asymmetry is why embargoes are never fully enforceable and why some operators prefer not to appear in WAN transit logs at all.

### Travel as a Pressure System

Movement within Charted Space is not free. Every standard route has a maintaining power that charges transit fees, conducts inspections, and reserves the right to refuse passage. Permits for advanced technology, weapons, biological materials, and pre-collapse artifacts may require approval from multiple jurisdictions when a route crosses polity borders, and the approvals do not always arrive before departure windows close.
...
The most consistent feature of travel in the Five Galaxies is that the cost, risk, and availability of transit are set by whoever controls the infrastructure between origin and destination; that control is always conditional, always contested, and always worth money to the right buyer.
